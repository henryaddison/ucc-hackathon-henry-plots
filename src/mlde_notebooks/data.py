...
def prep_eval_and_model_data(data_configs, split, ensemble_members, samples_per_run):
    models = {
        source: dict(
            sorted(
                {
                    ...
                    | run_config
                    for run_config in data_config
                }.items(),
                key=lambda x: x[1]["order"],
            )
        )
        for source, data_config in data_configs.items()
    }

    merged_ds = {
        source: prep_eval_data(
            data_config,
            split,
...
def si_to_mmday(da: xr.DataArray):
